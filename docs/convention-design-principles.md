...
### 10.7 Code を canonical とする doc dedup pattern
...
**適用場面**:
- ゲーム/システム定数 table (`ARENA_RADIUS = 20` 等) が `constants.ts` (JSDoc 付き) と重複
- Message schema table が TypeScript type union と重複
- Component file-by-file description が各 file の header comment と重複

**pattern**:
- 値・型の table を doc から削除
- 「canonical は `src/X.ts` (JSDoc + section コメントで分類)」という 2 行 pointer に置換
- category 名リストが必要な時は値なしで列挙 (e.g., 「Arena / Stardust / Exhaust / Energy 系に分類」)

**2026-04-18 LorentzArena 実証**: 2+1/CLAUDE.md から Parameters table (80 行) を削除し `constants.ts` pointer に。Message type table は docs/architecture.md に移管の上「canonical は `src/types/message.ts`」明記。

### 10.8 CLAUDE.md chain の nested auto-load (Claude Code 実装依存)
...
### 10.9 「超要約 (super-summary)」pattern
...
| 2026-04-18 | §10.7-10.10 追加 (Level-2 migration insights) | LorentzArena 2+1/CLAUDE.md の radical delegation (364 → 97 lines) から抽出: §10.7 code を canonical とする doc dedup (parameter/schema table を code pointer 化)、§10.8 CLAUDE.md chain の nested auto-load (Claude Code 特有、sub-project で chain が積み上がる)、§10.9 「超要約」pattern (slim CLAUDE.md に 5-8 項目×1行の 2 層化)、§10.10 migration level 階段 (Level 0-3)。LorentzArena chain 505 → 238 lines の実証値 |
