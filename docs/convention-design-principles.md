...
- 「原則に反する記述が他に無いか」 → **proxy keyword 単独で grep せず、 否定 keyword も複数 + コード/散文の両方で再 sweep**。 単一 keyword で sweep すると「同じことを別表現で書いた箇所」 を見逃す。 具体: 2026-06-05「全シート把握原則 (read は hidden を skip しない)」 を立てた後 `continue`/`sheet_state` で sweep し「もう無い」 とした直後に、 本文「各 **visible** sheet を inspect」 (= 同じ hidden 除外を別表現で書いた箇所) を見逃した RCA。 → 原則を立てたら **その否定 keyword (`visible` / `active` / 「のみ」 等) でも**再 sweep する。
