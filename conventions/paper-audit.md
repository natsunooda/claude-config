...
実装例: `ejp-revision/scripts/audit-forward-refs.py` (= 二例目で `claude-config/scripts/` に generic 化予定、 現状 ejp-revision 専用)。
...
## 実例: ejp-revision (2026-05-19、 1 day で完了)

- paper 39p → **37p** (-119 行)、 **13 findings** (= 12 implemented + 1 rejected)
- Phase 1 script: `ejp-revision/scripts/audit-forward-refs.py`
- findings DB: `ejp-revision/plans/2026-05-19-findings.yaml`
- plan: `ejp-revision/plans/2026-05-19-forward-ref-audit.md`
- 詳細: `ejp-revision/DESIGN.md §6`
...
将来別 paper で同様の audit を実施したら、 script を `claude-config/scripts/` に generic 化、 本 convention を refine。 現状は ejp-revision で完結。
