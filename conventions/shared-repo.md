...
「依存」 と「名指し (mention)」 の区別は重要 (= **collaborator が access を要求されるか** の有無):

- ❌ **依存** = collaborator が L3 にアクセスしないと本リポが standalone で成立しない (= 規約違反、 collaborator 環境で破綻)
- ✅ **名指し** = collaborator は本リポ範囲で完結できるが、 全体像理解のため upstream の存在を informational に明記 (= 後述 § boundary 明示付き名指し)

CLAUDE.md / DESIGN.md / README.md など共同編集者が触れるファイルに以下を **絶対に書かない (= 依存形)**:

- 所有者の個人層内ファイルへのファイルパス参照 (= layer 3 への literal path、 collaborator が clone できない / navigate できない)
...
#### 例外: boundary 明示付き名指し (条件付き許容)

所有者の他の private repo の **名前を出すこと自体は許容**する、 ただし以下の boundary 条件を **同じ場所で明示** すること:

1. **boundary 文を併記**: 「collaborator は本リポ範囲で完結 / 上記 upstream に直接アクセスする必要は無い」 等の文を `()` 内 or 同 bullet 内に書く (= 名指しを見た collaborator が「これは self-contained だ」 と即判断できる)
2. **L3 file path は書かない**: 名指しが OK なのは **repo 名のみ** (例: `physics-research`)、 内部 file path (例: `physics-research/papers/talks.yaml`) は依存形に格上げされるため別途許可が要る
3. **責務境界 / 取り込み手順 等 explanatory section に限る**: 通常の運用文書 (= setup 手順 / build 手順) 内に L3 名を書くと、 collaborator が必要だと誤読する。 explanatory 文脈に限定

**Why naming is OK**: claude-config の setup.sh 等は L1 → L3 名 (= `<owner>-prefs/`) を runtime 検出するし、 personal-layer.md は L1 から L3 概念を概説している。 名指し ≠ access dependency。 collaborator は「あ、 別 repo がある、 でも自分は本リポだけ触れば OK」 を理解できる方が、 abstract paraphrase (「odakin が別管理」) で何があるかわからないより親切。

**反対側の risk**: boundary 文を省くと collaborator が「physics-research を clone しないと動かないのか?」 と誤読し、 404 / permission denied で confused になる。 boundary 文の併記が absolute requirement。
...
#   名指しは許容 (= 上記 §「例外: boundary 明示付き名指し」 参照)
