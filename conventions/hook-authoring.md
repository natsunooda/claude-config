...
- hook の存在を「visible」 と reflex で扱わない、 定期 audit (= §2 の 3 軸 audit) で動作を継続確認する運用 mechanism が必要
...
- **(P1) 現時点の hook 健全性**: 該当 hook 全件で §2 の 3 軸 audit (= symlink + settings.json + try-fire) が pass
