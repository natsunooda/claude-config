...
### §8 sweep の follow-up: hook description と template 整合

**契機**: 上記 fix 直後に「他に直すとこは？」で re-sweep。grep で memory/feedback 文脈を総ざらいし 3 件発見。いずれも 2026-04-17 の §8 方針変更 (ask → deny + escape hatch) の波及漏れ。

**実装**:
- DESIGN.md §「hooks/ の役割分担」table: `memory-guard.sh`「§2 判別強制」→「`permissionDecision=deny` + `<!-- machine-local: -->` marker escape hatch（§8.3/§8.7）」、`memory-guard-bash.sh`「警告のみ」→「同じく deny + `machine-local` 文字列 escape hatch」（「警告のみ」は事実と逆だった — 実装は deny）
- CLAUDE.md 構造ツリーのコメント 2 行: 同じく §8 feedback deny + escape hatch に更新
- `templates/personal-layer/CLAUDE.md.template`: 「揮発的な『現在の作業プロジェクト』状態は MEMORY.md 側で管理」→「personal `CLAUDE.md` 下に `### 現在の作業プロジェクト` セクションで列挙（git 同期先、MEMORY.md は index-only: §8.7）」。前者は素直に実行すると `memory-guard.sh` で deny される矛盾テンプレだった

**LESSON**: hook の動作変更 (ask→deny) と description の更新を同じコミットで揃えるべきだった。source-of-truth (hook 実装) と description (DESIGN.md / CLAUDE.md / template) の同期は機械的チェックが難しいので、方針変更時は hook 動作を grep で確認してから description を再生成する。
