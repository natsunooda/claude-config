...
**Why**: もともと `setup.sh:863` で `SECRETS_REPOS=(secrets-config twcu-phys-lab)` を hardcode していたが、 `twcu-phys-lab` は所属機関名 (CLAUDE.md L105 「所属機関名」 禁止) で claude-config (= public layer 1) に持たせては駄目な literal。 `secrets-config` は L101 allow-list 内だが `twcu-phys-lab` は別 class の leak。 単に削除すると odakin の運用が壊れる (= 該当 repo の secrets が symlink されなくなる) ため、 値の正本を個人層に移動して `setup.sh` は path 経由で読み取る mechanism 化。
