...
│   ├── tool-call-robustness.md # Claude の tool call が「malformed and could not be parsed」 で壊れるのを防ぐ (= Bash 内の特殊文字密集 〔heredoc/多段パイプ/引用符ネスト〕 や 長文+表+絵文字 直後の tool call が serialization 干渉で崩れる、 対策 = 複雑ロジックは Write でファイル化して単純コマンド実行 / tool call ターンは本文プレーン / cd を compound に入れず git -C / commit msg は -F file、 2026-06-05 RCA、 hook-authoring §1 の bash 3.2 parser bug とは別 layer)
...
8. 全リポに pre-commit hook をインストール（Unicode→LaTeX 自動修正）— hook 自体が staged file に `.tex/.bib/.bst/.cls/.sty` が無ければ no-op で exit 0 するので、 LaTeX file 不在の repo にも install して問題ない。 旧方式 (LaTeX file 検出経由) は時点依存で、 setup.sh 実行後に `.tex` 追加された repo で hook 未 install のまま事故になっていた (2026-05-14 RCA は `DESIGN.md` 参照)
